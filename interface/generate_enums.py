...
class DBusXMLEnumParser:
    ...
    def __init__(self, xml_data, prefix, infile, outfile):
        ...
        self._writer.write(
            f"/* Autogenerated from {pathlib.Path(infile).name} */\n\n")
    ...
    def print_enum_impl(self):
        ...
        camel_case_name = ''.join([a.capitalize()
                                  for a in full_name.split('_')])
        ...
        for val in self._enum_data["values"]:
            dashified = "-".join(val["name"]
                                 [len(self._enum_data['name']) + 1:].lower().split("_"))
            triad = [f"{self.prefix.upper()}_{val["name"]}",
        ...
        if self._enum_data['is_flags']:
            self._writer.write(f"        g_flags_register_static (g_intern_static_string (\"{camel_case_name}\"), values);\n")
        else:
            self._writer.write(f"        g_enum_register_static (g_intern_static_string (\"{camel_case_name}\"), values);\n")
        self._writer.write(
    ...
    def handle_start_element(self, name, attrs):
        if name == "tp:enum" or name == "tp:flags":
            self._enum_data = {"name": attrs['name'], "values": [
            ], "docstring": [], "is_flags":  name == "tp:flags"}
            ...
            self._enum_data["values"].append({"name": f"{self._enum_data["name"]}_{attrs["suffix"]}",
                                              "value": attrs['value'],
                                              "docstring": []})
